function Test-AccountActivity {
    ...
    process {
        ...
        # not.
        # A missing attribute reads as a mask with no bits set, which is what "enabled,
        # nothing special" looks like - the safe answer for a caller that only tests bits.
        $uac = ConvertTo-UACValue -Value $ADObject.userAccountControl -Default 0
