<#
.SYNOPSIS
    Conversion of raw LDAP attribute values into the form the checks consume.

.DESCRIPTION
    Everything adPEAS reads out of the directory passes through ConvertFrom-LDAPAttribute
    below: Invoke-LDAPSearch calls it once per attribute per object, and every check module
    sees only what it returns.

    This used to live inline in Invoke-LDAPSearch's result loop, together with the three
    Convert-* helpers above it, which were declared inside its process block. Nothing could
    test any of it: reaching the conversion meant calling Invoke-LDAPSearch itself, which
    needs a real System.DirectoryServices.Protocols.SearchResponse - and SearchResultEntry
    has no public constructor. The helpers were reachable from the conversion only through
...
.NOTES
    ...
    Requires: adPEAS-OIDs.ps1 (Convert-OIDsToNames, ConvertFrom-OID), ConvertFrom-SID.ps1,
              ConvertFrom-SecurityDescriptor.ps1, ConvertFrom-KeyCredentialLink.ps1,
...
<#
.SYNOPSIS
    Converts one raw LDAP attribute value into the form the checks consume.

.DESCRIPTION
    A wrong conversion here is not a defect in one module but in all of them at once, and it
    is invisible from above, because a check only ever sees the converted value - never the
    raw one. That is the reason this is a function of its own: name and raw value in, the
    properties to set out, drivable from a table without an LDAP connection.

    The body is the original block, moved unchanged apart from two mechanical rewrites: the
    Add-Member calls became assignments into $ConvertedProperties, and the 'continue' that
    ended each multi-value branch became 'return $ConvertedProperties'. The variables
    $PropName / $PropNameLower / $PropValue are rebuilt under their original names rather
    than renaming roughly two hundred references.

    The accumulator is deliberately not called $result: the extensionData branch keeps a
    local $result of its own and would overwrite it mid-branch.

.PARAMETER Name
    The attribute name exactly as the directory returned it. It decides the output property
    name; the branch matching below is case-insensitive throughout.

.PARAMETER Value
    The raw value collection for that attribute - an array of strings, or of byte[] for
    binary attributes. A lone byte[] is accepted and wrapped.

.OUTPUTS
    [ordered] dictionary of property name -> converted value. Three shapes occur and all
    three matter:
      - one entry, the ordinary case
      - no entry at all, when the raw value carried nothing usable (an empty sIDHistory, a
        msDS-KeyCredentialLink that parsed to nothing). That is not the same as a null
        value, and the caller must not invent a property for it
      - three entries for msLAPS-Password, which also yields msLAPS-Account and
        msLAPS-Updated out of the same JSON blob

.EXAMPLE
    ConvertFrom-LDAPAttribute -Name 'userAccountControl' -Value @('512')
    Returns @{ userAccountControl = @('NORMAL_ACCOUNT') }
#>
function ConvertFrom-LDAPAttribute {
    [CmdletBinding()]
    param(
    ...
    # Normalise to the array shape the body expects: an Object[] whose elements are the
    # individual raw values, exactly what Invoke-LDAPSearch builds for an attribute.
    #
    # Written as an if STATEMENT with an assignment per branch, deliberately. As an
    # expression - $PropValue = if (...) { ,$Value } - the captured output is unrolled again
    # and the comma is undone: a byte[] comes back out as a byte[] of its own bytes, and a
    # string as a string, so $PropValue[0] then yields the first byte or the first character
    # instead of the value. Every branch below reads $PropValue[0].
    if ($null -eq $Value) {
        $PropValue = @()
    } elseif ($Value -is [byte[]]) {
        # byte[] is itself an array, so a lone binary value has to be wrapped explicitly -
        # @($bytes) would unroll it into single bytes and every byte[] branch would miss.
        $PropValue = ,$Value
    ...
    # Ordered, so properties reach the object in the order the conversion produced them.
    $ConvertedProperties = [ordered]@{}
        # =====================================================================
        ...
        if ($PropName -ieq "sIDHistory") {
            ...
            if ($sidStrings.Count -eq 1) {
                $ConvertedProperties[$PropName] = $sidStrings[0]
            } elseif ($sidStrings.Count -gt 1) {
                $ConvertedProperties[$PropName] = $sidStrings
            }
        ...
        if ($PropName -ieq "userCertificate" -or $PropName -ieq "cACertificate") {
            ...
            if ($certInfos.Count -eq 1) {
                $ConvertedProperties[$PropName] = $certInfos[0]
            } elseif ($certInfos.Count -gt 1) {
                $ConvertedProperties[$PropName] = $certInfos
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "msDS-KeyCredentialLink") {
            ...
            if ($keyInfos.Count -gt 0) {
                $ConvertedProperties[$PropName] = $keyInfos
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "msLAPS-EncryptedPasswordHistory") {
            ...
            try {
                ...
                if ($historyEntries.Count -gt 0) {
                    $ConvertedProperties[$PropName] = $historyEntries
                }
            } catch {
                $ConvertedProperties[$PropName] = "[History: $($PropValue.Count) entries]"
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "protocolSettings") {
            ...
            if ($protoLines.Count -eq 1) {
                $ConvertedProperties[$PropName] = $protoLines[0]
            } elseif ($protoLines.Count -gt 1) {
                $ConvertedProperties[$PropName] = $protoLines
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "pKIExtendedKeyUsage") {
            ...
            if ($ekuNames.Count -eq 1) {
                $ConvertedProperties[$PropName] = $ekuNames[0]
            } elseif ($ekuNames.Count -gt 1) {
                $ConvertedProperties[$PropName] = $ekuNames
            } else {
                $ConvertedProperties[$PropName] = @("Any Purpose")
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "msPKI-Certificate-Application-Policy") {
            ...
            if ($policyNames.Count -eq 1) {
                $ConvertedProperties[$PropName] = $policyNames[0]
            } elseif ($policyNames.Count -gt 1) {
                $ConvertedProperties[$PropName] = $policyNames
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -iin @("msPKI-RA-Application-Policies", "msPKI-RA-Policies")) {
            ...
            if ($raNames.Count -eq 1) {
                $ConvertedProperties[$PropName] = $raNames[0]
            } elseif ($raNames.Count -gt 1) {
                $ConvertedProperties[$PropName] = $raNames
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "pKICriticalExtensions") {
            ...
            if ($critExtNames.Count -eq 1) {
                $ConvertedProperties[$PropName] = $critExtNames[0]
            } elseif ($critExtNames.Count -gt 1) {
                $ConvertedProperties[$PropName] = $critExtNames
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -iin @("thumbnailPhoto", "jpegPhoto")) {
            if ($PropValue.Count -ge 1 -and $PropValue[0] -is [byte[]]) {
                ...
                $ConvertedProperties[$PropName] = $imageObj
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "mSMQDigests") {
            ...
            if ($digestValues.Count -eq 1) {
                $ConvertedProperties[$PropName] = $digestValues[0]
            } elseif ($digestValues.Count -gt 1) {
                $ConvertedProperties[$PropName] = $digestValues
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "mSMQSignCertificates") {
            ...
            if ($certValues.Count -eq 1) {
                $ConvertedProperties[$PropName] = $certValues[0]
            } elseif ($certValues.Count -gt 1) {
                $ConvertedProperties[$PropName] = $certValues
            }
            return $ConvertedProperties
        }
        ...
        if ($PropName -ieq "extensionData") {
            ...
            if ($decodedValues.Count -eq 1) {
                $ConvertedProperties[$PropName] = $decodedValues[0]
            } elseif ($decodedValues.Count -gt 1) {
                $ConvertedProperties[$PropName] = $decodedValues
            }
            return $ConvertedProperties
        }
        ...
        if ($PropValue.Count -eq 1) {
            ...
            if ($PropValue[0] -is [byte[]]) {
                if ($PropName -ieq "objectSid") {
                    try {
                        $ConvertedProperties[$PropName] = (New-Object System.Security.Principal.SecurityIdentifier($PropValue[0], 0)).Value
                    } catch {
                    ...
                    try {
                        $ConvertedProperties[$PropName] = ([System.Guid]$PropValue[0]).ToString()
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($GenIdBytes.Length -eq 16) {
                            ...
                            $ConvertedProperties[$PropName] = $GenIdGuid.ToString()
                        } elseif ($GenIdBytes.Length -eq 8) {
                            $HexString = ($GenIdBytes | ForEach-Object { $_.ToString('X2') }) -join '-'
                            $ConvertedProperties[$PropName] = $HexString
                        } else {
                            $HexString = ($GenIdBytes | ForEach-Object { $_.ToString('X2') }) -join ''
                            $ConvertedProperties[$PropName] = "0x$HexString"
                        }
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $SDResult
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($PasswordIdBytes -is [byte[]] -and $PasswordIdBytes.Length -ge 40 -and
                            ...
                            $ConvertedProperties[$PropName] = "RootKeyId: $($RootKeyId.ToString()), L0/L1/L2: $L0Index/$L1Index/$L2Index"
                    ...
                    try {
                        ...
                        if ($GuidBytes -is [byte[]] -and $GuidBytes.Length -eq 16) {
                            ...
                            $ConvertedProperties[$PropName] = $GuidObj.ToString()
                        } else {
                            $ConvertedProperties[$PropName] = $PropValue[0]
                        }
                    } catch {
                    ...
                    try {
                        ...
                        if ($rbcdPrincipals.Count -eq 1) {
                            $ConvertedProperties[$PropName] = $rbcdPrincipals[0]
                        } elseif ($rbcdPrincipals.Count -gt 1) {
                            $ConvertedProperties[$PropName] = $rbcdPrincipals
                        } else {
                            ...
                            $ConvertedProperties[$PropName] = "[SD present, no Allow ACEs]"
                        }
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        $bytes = $PropValue[0]
                        $hexStr = ($bytes | ForEach-Object { $_.ToString('X2') }) -join ''
                        $ConvertedProperties[$PropName] = $hexStr
                    } catch {
                        $ConvertedProperties[$PropName] = "[Hash: $($PropValue[0].Length) bytes]"
                    }
                    ...
                    try {
                        ...
                        if ($bytes -is [byte[]] -and $bytes.Length -eq 21) {
                            ...
                            if ($allAllowed) {
                                $ConvertedProperties[$PropName] = "All hours (no restriction)"
                            } else {
                                ...
                                if ($allDenied) {
                                    $ConvertedProperties[$PropName] = "No hours allowed (logon blocked)"
                                } else {
                                    ...
                                    $ConvertedProperties[$PropName] = $restrictions
                                }
                            }
                        } else {
                            # Unexpected format - show as hex
                            $hexStr = ($bytes | ForEach-Object { $_.ToString('X2') }) -join ''
                            $ConvertedProperties[$PropName] = "[LogonHours: $hexStr]"
                        }
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($Bytes.Length -eq 8) {
                            ...
                            if ($Seconds -ge 31536000) {
                                ...
                                $ConvertedProperties[$PropName] = "$Years year(s)"
                            } elseif ($Seconds -ge 86400) {
                                $Days = [Math]::Floor($Seconds / 86400)
                                $ConvertedProperties[$PropName] = "$Days day(s)"
                            } elseif ($Seconds -ge 3600) {
                                $Hours = [Math]::Floor($Seconds / 3600)
                                $ConvertedProperties[$PropName] = "$Hours hour(s)"
                            } else {
                                $Minutes = [Math]::Floor($Seconds / 60)
                                $ConvertedProperties[$PropName] = "$Minutes minute(s)"
                            }
                        } else {
                            $ConvertedProperties[$PropName] = $PropValue[0]
                        }
                    ...
                    try {
                        ...
                        if ($Bytes.Length -ge 2) {
                            ...
                            if ($KeyUsageList.Count -gt 0) {
                                $ConvertedProperties[$PropName] = $KeyUsageList
                            } else {
                    ...
                    if ($tsLines) {
                        if ($tsLines.Count -eq 1) {
                            $ConvertedProperties[$PropName] = $tsLines[0]
                        } else {
                            $ConvertedProperties[$PropName] = $tsLines
                        }
                    } else {
                        $byteLen = if ($PropValue[0] -is [byte[]]) { $PropValue[0].Length } else { 0 }
                        $ConvertedProperties[$PropName] = "[userParameters blob: $byteLen bytes, no recognised TSPropertyArray signature]"
                    }
                    ...
                    if ($licLines) {
                        if ($licLines.Count -eq 1) {
                            $ConvertedProperties[$PropName] = $licLines[0]
                        } else {
                            $ConvertedProperties[$PropName] = $licLines
                        }
                    } else {
                        $byteLen = if ($PropValue[0] -is [byte[]]) { $PropValue[0].Length } else { 0 }
                        $ConvertedProperties[$PropName] = "[terminalServer blob: $byteLen bytes, unknown format]"
                    }
                } else {
                ...
                if ($Script:LDAPFileTimeAttributes.Contains($PropNameLower)) {
                    ...
                    try {
                        ...
                        if ($ftValue -eq 0 -or $ftValue -eq 9223372036854775807) {
                            ...
                            $ConvertedProperties[$PropName] = $dateTime
                        }
                    } catch {
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $UACFlags
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($EncTypeFlags.Count -gt 0) {
                            $ConvertedProperties[$PropName] = $EncTypeFlags
                        } else {
                            $ConvertedProperties[$PropName] = @("Not configured")
                        }
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $SamTypeName
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $GroupTypeName
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($InstType -eq 4) {
                            $ConvertedProperties[$PropName] = "Writable"
                        } else {
                            $InstTypeFlags = @()
                            ...
                            $ConvertedProperties[$PropName] = ($InstTypeFlags -join ", ")
                        }
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($SysFlagsArray.Count -gt 0) {
                            $ConvertedProperties[$PropName] = $SysFlagsArray
                        } else {
                            $ConvertedProperties[$PropName] = @("NONE")
                        }
                    } catch {
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $TrustDirName
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $TrustTypeName
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($TrustAttrFlags.Count -gt 0) {
                            $ConvertedProperties[$PropName] = $TrustAttrFlags
                        } else {
                            $ConvertedProperties[$PropName] = @("NONE")
                        }
                    } catch {
                    ...
                    try {
                        ...
                        if ($PwdPropsFlags.Count -gt 0) {
                            $ConvertedProperties[$PropName] = $PwdPropsFlags
                        } else {
                            $ConvertedProperties[$PropName] = @("NONE")
                        }
                    } catch {
                ...
                # a cleanup.
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $ExpirationDateTime
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($LAPSObj.p) {
                            $ConvertedProperties[$PropName] = $LAPSObj.p
                            if ($LAPSObj.n) {
                                $ConvertedProperties['msLAPS-Account'] = $LAPSObj.n
                            }
                            if ($LAPSObj.t) {
                                try {
                                    $LAPSFileTime = [Convert]::ToInt64($LAPSObj.t, 16)
                                    $ConvertedProperties['msLAPS-Updated'] = ([DateTime]::FromFileTimeUtc($LAPSFileTime))
                                } catch { }
                            }
                        } else {
                            $ConvertedProperties[$PropName] = $PropValue[0]
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $DisplayTypeName
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $TypeName
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        if ($CertNameFlags.Count -gt 0) {
                            $ConvertedProperties[$PropName] = $CertNameFlags
                        } else {
                            $ConvertedProperties[$PropName] = @("NONE")
                        }
                    } catch {
                    ...
                    try {
                        ...
                        if ($EnrollFlags.Count -gt 0) {
                            $ConvertedProperties[$PropName] = $EnrollFlags
                        } else {
                            $ConvertedProperties[$PropName] = @("NONE")
                        }
                    } catch {
                    ...
                    try {
                        ...
                        if ($PrivKeyFlags.Count -gt 0) {
                            $ConvertedProperties[$PropName] = $PrivKeyFlags
                        } else {
                            $ConvertedProperties[$PropName] = @("NONE")
                        }
                    } catch {
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $KeySpecName
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $IPAddr.ToString()
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $VersionString
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                    ...
                    try {
                        ...
                        $ConvertedProperties[$PropName] = $QuotaString
                    } catch {
                        $ConvertedProperties[$PropName] = $PropValue[0]
                    }
                } else {
